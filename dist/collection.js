...
router.get('/', async function (req, res, next) {
  req.urApi.query('collections.getSummary').then(async function (result) {
    try {
      ...
      let valeurs = {}; // cartes.forEach(async (item) => {
...
router.get('/doubles/:montant?', async (req, res, next) => {
  try {
    ...
    const summary = (await req.urApi.query("collections.getSummary")).items; // console.log(summary);
    ...
    const totalCartes = summary.reduce((memo, {
      nbOwnedTotal
    }) => memo + nbOwnedTotal, 0);
    ...
    const doublesPairs = _.pairs(_.pick(_.countBy(pagesPlates, ({
      id
    }) => id), value => value >= 2)); // console.log(doublesPairs);


    const doubles = doublesPairs.map(elt => ({
      id: elt[0],
      nb: elt[1]
    })); // const ids = ;

    const prix = _.flatten((await Promise.all(doubles.map(({
      id
    }) => req.urApi.query("market.getCharactersPricesCurrent", {
      charactersIDs: id
    })))).map(reponse => reponse.items));

    const persos = (await req.urApi.query("characters.getCharacters")).items; // console.log(prix);

    const valeurs = doubles.map(({
      id,
      nb
    }) => ({
      id: parseInt(id),
      prix: nb * prix.find(elt => parseInt(id) === elt.id).min,
      name: persos.find(elt => parseInt(id) === elt.id).name
    }));
    const valeursTri = valeurs.sort((a, b) => b.prix - a.prix);
    const valeursTriPallier = valeurs.filter(({
      prix
    }) => prix >= min); // console.log((await req.urApi.query("characters.getCharacters")).items);
    // console.log(summary);
    ...
    res.render('listedoubles', {
      doubles: valeursTriPallier,
      default: req.default
    });
  } catch (error) {
    console.error(error);
...
router.get('/listesemi', async (req, res, next) => {
  try {
    const semis = await recupSemiEvo(req.urApi); // let listeclans = semis.map(({clan_name, clan_id}) => ({clan_name, clan_id}));
    // listeclans = _.uniq(listeclans);

    const listeclans = (await req.urApi.query("characters.getClans")).items; // console.log(listeclans);

    res.render('listesemi', {
      listeclans,
      semis,
      default: req.default
    });
...
router.get('/annee', async (req, res, next) => {
  const cartes = await req.urApi.query('characters.getCharacters', {
    maxLevels: true
  });
  const filtrees = cartes.items.filter(value => {
    const date = new Date(Number(value.release_date) * 1000); // date.setTime(value.release_date);

    console.log(date.getFullYear());
    return date.getFullYear() === 2018; // return console.log(value);
  });
...
router.get('/rarities', async (req, res, next) => {
  ...
  try {
    const {
      items
    } = await urApi.query('characters.getCharacters', {
      maxLevels: true
    }); // console.log(items);

    const clans = {};

    const clansGroups = _.groupBy(items, ({
      clan_name
    }) => clan_name);

    for (const clansKey in clansGroups) {
      const raritiesGroup = _.groupBy(clansGroups[clansKey], ({
        rarity
      }) => rarity);
...
router.get('/listePouvoirsUniques', async (req, res, next) => {
  ...
  try {
    ...
    const persosPouvoirsUniques = _.filter(items, item => pouvoirsUniques.includes(item.ability)); // console.log(persosPouvoirsUniques);


    const listeclans = (await urApi.query("characters.getClans")).items;
    res.render('listesemi', {
      listeclans,
      semis: persosPouvoirsUniques,
      default: req.default
    });
  } catch (error) {
    console.error(error);
...
async function recupSemiEvo(urApi) {
  ...
  const semiPersos = items.filter(perso => perso.level_max > perso.ability_unlock_level && perso.ability_id !== 0 || perso.level_min === 1 && perso.power >= 6); // console.log(semiPersos);

  const semiNiveauxResultats = await Promise.all(semiPersos.map(({
    id,
    level_max
  }) => urApi.query("characters.getCharacterLevels", {
    characterID: id,
    levelMax: level_max - 1
  })));
  const semiNiveauxCartes = semiNiveauxResultats.map(({
    items
  }) => items);
  const test = semiNiveauxCartes.filter(entree => entree.id === 140); // console.log(test);

  const semiNiveauxMerged = [].concat.apply([], semiNiveauxCartes);
  const semiNiveauxSeuls = semiNiveauxMerged.filter(({
    level,
    ability_unlock_level,
    level_max,
    power,
    ability_id
  }) => level_max > ability_unlock_level && level >= ability_unlock_level && ability_id !== 0 || level === 1 && power >= 6);
...
async function recupFaibleDiff(urApi) {
  ...
  const infosCartesFD = await Promise.all(items.map(({
    id
  }) => urApi.query("characters.getCharacterLevels", {
    characterID: id
  })));
  const listeFiltree = infosCartesFD.map(({
    items
  }) => {
    let precedent = [];
    ...
    for (const element of items) {
      const score = totalScorePerso(element); // if(element.id === 418) {
      //   // console.log(element);

...
router.get('/clearpresets', async (req, res, next) => {
  try {
    ...
    const {
      items
    } = await req.urApi.query("collections.getPresets"); // Lorsque je la reçois, je fais une requête pour supprimer chaque preset.
    // J'attends que les promesses se soient toutes exécutées.

    await Promise.all(items.map(({
      id
    }) => req.urApi.query("collections.deleteSavedPreset", {
      presetID: id
    })));
    res.redirect('/user/');
  } catch (err) {
